...
// One file per job. Every one of these is an `impl MelonEgui` block over the
// same struct, split by *what the methods are for* rather than by type — the
// front end is one object, and pretending otherwise would mean threading it
// through a dozen constructors for no gain.
mod boot;
mod cheat_codes;
mod commands;
mod dialogs;
