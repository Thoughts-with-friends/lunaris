...
export const registerDocumentSemanticTokensProvider = (
  monacoEnv: typeof monaco,
) => {
  monacoEnv.languages.registerDocumentSemanticTokensProvider(
    HKANNO_LANGUAGE_ID,
    {
      getLegend: () => ({
        tokenTypes: TOKEN_TYPES.slice(),
        ...
        for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
          ...
          switch (node.kind) {
            ...
            case "motion": {
              ...
              [node.x, node.y, node.z].forEach((n) =>
                pushToken(n?.pos, "number"),
              );
              break;
            }
