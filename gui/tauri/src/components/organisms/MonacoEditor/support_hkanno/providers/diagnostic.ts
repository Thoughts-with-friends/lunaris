...
import { type IFrameNode } from "../parser/strict/nodes";
...
const updateHkannoDiagnostics: OnMount = (editor, monacoEnv) => {
  ...
  for (let lineNumber = 1; lineNumber <= lines.length; lineNumber++) {
    ...
    if (node.kind == "iframe") {
      markers.push(
        ...provideIFrameDiagnostics(node, lineNumber, line.length, monacoEnv),
      );
    }
    ...
    if (node.kind === "payload_instruction") {
      ...
      if (name) {
        const def = PIE_NATIVE_INSTRUCTIONS.find(
          (i) => i.name.toUpperCase() === name,
        );
        if (def) {
          const provided =
            pieNode.instruction?.parameters?.items.filter((item) => item.value)
              .length ?? 0;
    ...
    if (node.kind === "text") {
      if (node.time && !node.text?.value) {
        const startCol =
          node.space1TimeToText?.pos?.startColumn ??
          node.time?.pos?.endColumn ??
          1;
