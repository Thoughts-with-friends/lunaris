    ...
    provideSignatureHelp(model, position) {
      ...
      if (node.kind === "motion") {
        const activeParameter: 0 | 1 | 2 = (() => {
          ...
          if (node.y?.pos === undefined || node.y.pos.endColumn >= cursorCol)
            return 1;
          if (node.z?.pos === undefined || node.z.pos.endColumn >= cursorCol)
            return 2;
          return 2;
        })();
      ...
      if (node.kind === "text") {
        if (!node.time || !node.space1TimeToText) {
          return valueOf(
            "<time: f32>",
            "Timestamp in seconds (e.g., 0.100000)",
            "time",
          );
        }
    ...
    signatures: [
      {
        ...
        parameters: [{ label: paramLabel, documentation: doc }],
      },
    ],
    activeSignature: 0,
    activeParameter: 0,
