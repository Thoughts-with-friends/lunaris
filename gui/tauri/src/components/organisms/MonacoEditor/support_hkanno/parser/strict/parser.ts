...
export const parseNumberField = (
  ...
  while (state.i < state.len && /[0-9.]/.test(state.line[state.i]))
    str += state.line[state.i++];

  if (
    state.i < state.len &&
    (state.line[state.i] === "e" || state.line[state.i] === "E")
  ) {
    str += state.line[state.i++];
    if (state.line[state.i] === "+" || state.line[state.i] === "-")
      str += state.line[state.i++];
...
const parseLiteralField = <T extends string>(
  state: ParserState,
  literal: T,
): FieldNode<T> | undefined => {
  const start = state.i;
 ...
 */
export const parseFieldUntil = (
  state: ParserState,
  stopChars: string,
): FieldNode<string> | undefined => {
  const start = state.i;
  let value = "";
...
export const parseTrackNameLine = (
  line: string,
  lineNumber = 1,
): TrackNameNode => {
  const state: ParserState = { line, i: 0, lineNumber, len: line.length };
 ...
 */
export const parseAnimMotionLine = (
  line: string,
  lineNumber = 1,
): MotionNode => {
  const state: ParserState = { line, i: 0, lineNumber, len: line.length };
 ...
 */
export function parseAnimRotationLine(
  line: string,
  lineNumber = 1,
): RotationNode {
  const state: ParserState = { line, i: 0, lineNumber, len: line.length };
...
const commonParsers = [
  ...
  {
    check: (line: string) =>
      line.trimStart().toLowerCase().startsWith("trackname:"),
...
const extParser = [
  {
    check: (line: string) =>
      line.toLowerCase().includes("specialframes_invincible"),
 ...
 */
export const parseHkannoLineExt = (
  line: string,
  lineNumber = 1,
): HkannoNodeExt => {
