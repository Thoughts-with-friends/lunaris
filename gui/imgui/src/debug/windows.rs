...
impl DebugWindowState for PalettesWindowState {
    ...
    fn render(&mut self, ui: &Ui) {
        ...
        ComboBox::new(im_str!("Engine")).build_simple(
            ui,
            &mut self.palettes_engine,
            &Self::ENGINES,
            &(|i| Cow::from(ImString::new(i.label()))),
        );

        ui.set_next_item_width(combo_width);
        ComboBox::new(im_str!("Graphics Type")).build_simple(
            ui,
...
impl DebugWindowState for TilesWindowState {
    ...
    fn render(&mut self, ui: &Ui) {
        ...
        if Self::ENGINES[self.tiles_engine] == Engine::A {
            Slider::new(im_str!("Offset"))
                .range(Self::TILES_RANGES[self.tiles_graphics_type].clone())
