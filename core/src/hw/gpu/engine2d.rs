...
impl<E: EngineType> Engine2D<E> {
        ...
        match self.dispcnt.bg_mode {
            BGMode::Mode0 => {
                ...
                if self.dispcnt.contains(DISPCNTFlags::DISPLAY_BG2) {
                    self.render_text_line(vram, vcount, 2)
                }
                if self.dispcnt.contains(DISPCNTFlags::DISPLAY_BG3) {
                    self.render_text_line(vram, vcount, 3)
                }
            ...
            BGMode::Mode3 => {
                ...
                if self.dispcnt.contains(DISPCNTFlags::DISPLAY_BG3) {
                    self.render_extended_line(vram, 3)
                }
                self.process_lines(vcount, 0, 3);
            }
            BGMode::Mode4 => {
            ...
            BGMode::Mode5 => {
                ...
                if self.dispcnt.contains(DISPCNTFlags::DISPLAY_BG2) {
                    self.render_extended_line(vram, 2)
                }
                if self.dispcnt.contains(DISPCNTFlags::DISPLAY_BG3) {
                    self.render_extended_line(vram, 3)
                }
    ...
    fn render_objs_line(&mut self, vram: &VRAM, vcount: u16) {
        ...
        let mut objs = oam_parsed
            .iter()
            .filter(|obj| {
                let obj_shape = (obj[0] >> 14 & 0x2) as usize;
        ...
        for dot_x in 0..GPU::WIDTH {
            ...
            for obj in objs.iter() {
                ...
                let (boundary, tile_offset) = if self.dispcnt.contains(DISPCNTFlags::TILE_OBJ_1D) {
                    (
                        32 << self.dispcnt.tile_obj_1d_bound,
                ...
                let (palette_num, color_num) = Engine2D::<E>::get_color_from_tile(
                    vram,
                    VRAM::get_obj::<E, u8>,
                    addr,
                    false,
        ...
        ) -> u16,
    {
        ...
        for dot_x in 0..GPU::WIDTH {
            ...
            self.bg_lines[bg_i][dot_x] = render_fn(
                self,
                vram,
                bg_i,
                map_start_addr,
    ...
    fn render_text_line(&mut self, vram: &VRAM, vcount: u16, bg_i: usize) {
        ...
        let inc_map_x = |map_x: &mut usize, map_start_addr_x_offset: &mut usize| {
            if *map_x + 1 >= 32 {
                assert_eq!(*map_x, 31);
                *map_x = 0usize;
        ...
        let start_x = if tile_x != 0 {
            ...
            let colors = self.render_16bit_entry_f(
                vram,
                bg_i,
                map_start_addr,
                tile_start_addr,
    ...
    pub(super) fn render_16bit_entry_f(
        &mut self,
        vram: &VRAM,
        bg_i: usize,
        map_start_addr: usize,
    ...
    pub(super) fn render_16bit_entry(
        &self,
        vram: &VRAM,
        bg_i: usize,
        map_start_addr: usize,
    ...
    pub(super) fn get_color_from_tile<F: Fn(&VRAM, usize) -> u8>(
        vram: &VRAM,
        get_vram_byte: F,
        addr: usize,
        flip_x: bool,
...
impl<E: EngineType> Engine2D<E> {
    pub fn read_register(&self, addr: u32) -> u8 {
        ...
        match addr & 0xFFF {
            ...
            0x013 => self.vofs[0].byte1(),
            0x014 => self.hofs[1].byte0(),
            0x015 => self.hofs[1].byte1(),
            0x016 => self.vofs[1].byte0(),
            0x017 => self.vofs[1].byte1(),
            0x018 => self.hofs[2].byte0(),
            0x019 => self.hofs[2].byte1(),
            0x01A => self.vofs[2].byte0(),
            0x01B => self.vofs[2].byte1(),
            0x01C => self.hofs[3].byte0(),
            0x01D => self.hofs[3].byte1(),
            0x01E => self.vofs[3].byte0(),
            0x01F => self.vofs[3].byte1(),
            ...
            0x056..=0x05F => 0,
            _ => {
                warn!("Ignoring Engine2D Read at 0x{:08X}", addr);
                0
            }
        }
    }
    ...
    pub fn write_register(&mut self, scheduler: &mut Scheduler, addr: u32, value: u8) {
        ...
        match addr & 0xFFF {
            ...
            0x013 => self.vofs[0].set_byte1(value),
            0x014 => self.hofs[1].set_byte0(value),
            0x015 => self.hofs[1].set_byte1(value),
            0x016 => self.vofs[1].set_byte0(value),
            0x017 => self.vofs[1].set_byte1(value),
            0x018 => self.hofs[2].set_byte0(value),
            0x019 => self.hofs[2].set_byte1(value),
            0x01A => self.vofs[2].set_byte0(value),
            0x01B => self.vofs[2].set_byte1(value),
            0x01C => self.hofs[3].set_byte0(value),
            0x01D => self.hofs[3].set_byte1(value),
            0x01E => self.vofs[3].set_byte0(value),
            0x01F => self.vofs[3].set_byte1(value),
    ...
    pub fn obj_palettes(&self) -> &Vec<u16> {
        &self.obj_palettes
    }
